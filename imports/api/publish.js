...
publishComposite('league', leagueId => ({
  find() {
  ...
  children: [
    {
      find(league) {
        return Meteor.users.find({ _id: { $in: league.usersInLeague } });
      },
    },
    {
      find(league) {
        return Player.find({
          _id: {
            $in: league.currentPlayerUpForBidId
              ? league.playersDrafted.concat(league.currentPlayerUpForBidId)
              : league.playersDrafted,
          },
        });
      },
    },
